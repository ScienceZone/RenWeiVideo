package com.ipmph.v.viewpagerindicator;
        ...
        for (int i = 0; i < N; i++) {
            ...
            switch (attr) {
                ...
                case R.styleable.IndicatorView_indicator_margin:
                    float defaultMargin = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,2,getResources().getDisplayMetrics());
                    mMargin = (int) typedArray.getDimension(attr , 0);
                    break ;
                case R.styleable.IndicatorView_indicator_smooth:
                    mSmooth = typedArray.getBoolean(attr,true) ;
                    break;
            }
        }
        //使用完成之后记得回收
