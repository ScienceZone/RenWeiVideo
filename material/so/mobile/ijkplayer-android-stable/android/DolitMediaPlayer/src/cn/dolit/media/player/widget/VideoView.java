...
import cn.dolit.media.player.DLMediaPlayer;
import cn.dolit.media.player.IMediaPlayer.OnBufferingUpdateListener;
import cn.dolit.media.player.IMediaPlayer.OnCompletionListener;
import cn.dolit.media.player.IMediaPlayer.OnErrorListener;
import cn.dolit.media.player.IMediaPlayer.OnInfoListener;
import cn.dolit.media.player.IMediaPlayer.OnPreparedListener;
import cn.dolit.media.player.IMediaPlayer.OnSeekCompleteListener;
import cn.dolit.media.player.IMediaPlayer.OnVideoSizeChangedListener;
import cn.dolit.media.player.R;
import cn.dolit.media.player.option.AvFormatOption_HttpDetectRangeSupport;
import cn.dolit.media.player.option.AvFourCC;
...
public class VideoView extends SurfaceView implements
    ...
    private void openVideo() {
        ...
        try {
            ...
            DLMediaPlayer dlMediaPlayer = null;
            if (mUri != null) {
                dlMediaPlayer = new DLMediaPlayer();
                int ret = dlMediaPlayer.Init(strPlayEngineKey1, strPlayEngineKey2, this.mContext);
                if (ret != 0)
                {
                	DebugLog.d(TAG, "Init DolitMediaPlayer engine failed, please check Key!");
                	return;
                }
                dlMediaPlayer.setAvOption(AvFormatOption_HttpDetectRangeSupport.Disable);
                dlMediaPlayer.setOverlayFormat(AvFourCC.SDL_FCC_RV32);

                dlMediaPlayer.setAvCodecOption("skip_loop_filter", "48");
                dlMediaPlayer.setFrameDrop(12);
                if (mUserAgent != null) {
                    dlMediaPlayer.setAvFormatOption("user_agent", mUserAgent);
                }
            }
            mMediaPlayer = dlMediaPlayer;
