...
/*
±ê×¼ÑÝÊ¾°æ£º
packageName£ºcn.dolit.encplayer.demo
...
const unsigned char certStr[] = {0x9, 0xc, 0x19, 0x7b, 0x02, 0x0, 0x0, 0x00, 0x00, 0x7, 
0xc, 0x96, 0x29, 0x73, 0x00, 0x00, '&', 'G', 0x4, 0x00, 0x77, 'H', 0x00, 0x97, '0', 0x00, 0x01};
...
const unsigned char certStr[] = {0x1, 0x2, 0x12, 0x3b, 0x42, 0x5, 0x6, 0x70, 0x80, 0x97, 
0x1c, 0x26, 0x39, 0x43, 0x50, 0x60, 'A', 'x', 0x14, 0x30, 0x77, 'z', 0x90, 0x97, 'z', 0x50, 0x31};

static int g_fdv_valid = 0;
void av_reg_fdv(int license)
{
   if (license == 0x135132)
      g_fdv_valid = 2;
}
void av_register_all(const unsigned char * ms)
{
    if (ms == NULL)
        return;
    ...
    if (initialized)
        return;
    if (memcmp(certStr, ms, sizeof(certStr) / sizeof(certStr[0])) != 0)
	return;
