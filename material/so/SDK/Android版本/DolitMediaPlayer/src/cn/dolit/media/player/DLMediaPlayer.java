...
import cn.dolit.media.player.annotations.AccessedByNative;
import cn.dolit.media.player.annotations.CalledByNative;
import cn.dolit.media.player.option.AvFormatOption;
import cn.dolit.media.player.pragma.DebugLog;
 ...
 */
public final class DLMediaPlayer extends SimpleMediaPlayer {
    private final static String TAG = DLMediaPlayer.class.getName();
     ...
     */
    private static DLLibLoader sLocalLibLoader = new DLLibLoader() {
        @Override
    ...
    public static void loadLibrariesOnce(DLLibLoader libLoader) {
        synchronized (DLMediaPlayer.class) {
            if (!mIsLibLoaded) {
                libLoader.loadLibrary("ijkffmpeg");
                libLoader.loadLibrary("util");
     ...
     * synchronously instantiating a DLMediaPlayer from a Uri or resource.
     * <p>
     * When done with the DLMediaPlayer, you should call {@link #release()}, to
     * free the resources. If not released, too many DLMediaPlayer instances
     * may result in an exception.
     * </p>
     */
    public DLMediaPlayer() {
     ...
     * @param reserved
     */
    public DLMediaPlayer(DLLibLoader libLoader) {
        initPlayer(libLoader);
    }

    private void initPlayer(DLLibLoader libLoader) {
         ...
         */
        native_setup(new WeakReference<DLMediaPlayer>(this));
    }
    ...
    /*
     * Update the DLMediaPlayer SurfaceTexture. Call after setting a new
     * display surface.
     */
    ...
    /**
     * Releases resources associated with this DLMediaPlayer object. It is
     * considered good practice to call this method when you're done using the
     * DLMediaPlayer. In particular, whenever an Activity of an application is
     * paused (its onPause() method is called), or stopped (its onStop() method
     * is called), this method should be invoked to release the DLMediaPlayer
     ...
     * if a DLMediaPlayer object is no longer needed may also lead to
    ...
    private native final void native_setup(Object DLMediaPlayer_this);

    private native final void native_finalize();

    private native final void native_message_loop(Object DLMediaPlayer_this);
    ...
    private static class EventHandler extends Handler {
        private WeakReference<DLMediaPlayer> mWeakPlayer;

        public EventHandler(DLMediaPlayer mp, Looper looper) {
            super(looper);
            mWeakPlayer = new WeakReference<DLMediaPlayer>(mp);
        }

        @Override
        public void handleMessage(Message msg) {
            DLMediaPlayer player = mWeakPlayer.get();
            if (player == null || player.mNativeMediaPlayer == 0) {
                DebugLog.w(TAG,
                        "DLMediaPlayer went away with unhandled events");
                return;
            }
     ...
     * thread. We use a weak reference to the original DLMediaPlayer object so
    ...
    private static void postEventFromNative(Object weakThiz, int what,
        ...
        DLMediaPlayer mp = (DLMediaPlayer) ((WeakReference) weakThiz).get();
        if (mp == null) {
            return;
        }
    ...
    private static int onControlResolveSegmentCount(Object weakThiz) {
        ...
        WeakReference<DLMediaPlayer> weakPlayer = (WeakReference<DLMediaPlayer>) weakThiz;
        DLMediaPlayer player = weakPlayer.get();
        if (player == null)
            return -1;
